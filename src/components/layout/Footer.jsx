  ...
  return (
    <footer className="w-full border-t border-[#222] px-6 lg:px-12 py-10 flex flex-col md:flex-row justify-between items-center md:items-end font-mono text-xs text-[#555] relative z-30 bg-[#030712] gap-10">
      ...
      <div className="flex flex-col items-center w-full md:w-1/3 gap-4 order-1 md:order-2">
        <div className="text-[#6366f1] tracking-[0.3em] font-bold uppercase opacity-80">{DATA.identity.model} // op_id_026</div>
        ...
        <div className="text-xs text-[#222] tracking-[0.6em] font-bold">2026_HEARTBEAT_PROTOCOL</div>
      </div>

      {/* Right: Build Info */}
      <div className="flex flex-col gap-1.5 items-center md:items-end w-full md:w-1/3 md:text-right order-3">
        <div className="text-[#666]">origin_node: {DATA.identity.location.toLowerCase().replace(/ /g, '_')}</div>
        <div className="text-[#666]">build_fingerprint: 0x8a2f...7e4c</div>
        <div className="text-[#666]">kernel_version: 5.10.104_dk_lts</div>
        <div className="text-[#6366f1] font-bold tracking-widest uppercase opacity-40">terminal_subsystem: online</div>
      </div>
    </footer>
  );
}
